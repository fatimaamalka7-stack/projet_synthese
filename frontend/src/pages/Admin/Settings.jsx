...
export default function AdminSettings() {
  ...
  const themeColors = [
    { name: 'Violet (défaut)', cls: 'bg-purple-500' },
    { name: 'Bleu',            cls: 'bg-blue-500' },
    { name: 'Rose',            cls: 'bg-pink-500' },
    { name: 'Émeraude',        cls: 'bg-emerald-500' },
      ...
      <section className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-6">
        ...
        <div>
          ...
          <div className="flex gap-3 flex-wrap">
            {themeColors.map(({ name, cls }) => (
              <button key={name} title={name}
                className={`w-9 h-9 rounded-full ${cls} ring-2 ring-offset-2 ring-offset-white dark:ring-offset-gray-800 ring-transparent hover:ring-gray-400 transition-all first:ring-primary-500`}/>
            ))}
          </div>
