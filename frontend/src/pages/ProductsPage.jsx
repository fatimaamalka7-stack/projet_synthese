...
export default function ProductsPage({ categoryId, title }) {
  const { t } = useTranslation()
  const pageTitle = title || t('nav.all_products')
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 animate-fade-in">
      <h1 className="font-display text-3xl font-bold mb-6">{pageTitle}</h1>
      ...
      <div className="flex flex-col md:flex-row gap-3 mb-8">
        <form onSubmit={handleSearch} className="flex gap-2 flex-1">
          ...
          <button type="submit" className="btn-primary whitespace-nowrap">{t('products.search_btn')}</button>
        </form>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-gray-500">{t('products.no_products_desc')}</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
