...
export default function CartPage() {
  ...
  if (!user) return (
    <div className="max-w-lg mx-auto px-4 py-20 text-center">
      ...
      <h2 className="font-display text-2xl font-bold mb-3">Votre panier est vide</h2>
      <p className="text-gray-500 mb-6">Connectez-vous pour accéder à votre panier</p>
  ...
  if (items.length === 0) return (
    <div className="max-w-lg mx-auto px-4 py-20 text-center">
      <p className="text-5xl mb-4">🛒</p>
      <h2 className="font-display text-2xl font-bold mb-3">Panier vide</h2>
      <p className="text-gray-500 mb-6">Découvrez nos produits et ajoutez-en au panier !</p>
      <Link to="/" className="btn-primary">Commencer les achats</Link>
    </div>
  )

  return (
    <div className="max-w-5xl mx-auto px-4 py-8 animate-fade-in">
      <h1 className="font-display text-3xl font-bold mb-6">Mon Panier ({itemCount} article{itemCount > 1 ? 's' : ''})</h1>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-5 h-fit sticky top-24">
          <h3 className="font-display text-xl font-bold mb-4">Résumé</h3>
          ...
          <button onClick={() => navigate('/checkout')} className="btn-primary w-full flex items-center justify-center gap-2">
            <FiShoppingBag size={16}/> Commander <FiArrowRight size={16}/>
          </button>
          <Link to="/" className="btn-secondary w-full text-center mt-3 block">Continuer les achats</Link>
        </div>
      </div>
    </div>
  )
