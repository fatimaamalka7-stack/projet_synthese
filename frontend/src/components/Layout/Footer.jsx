      ...
      <div className="max-w-6xl mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8">
        <div>
          ...
          <p className="text-sm leading-relaxed text-gray-400">
            Votre destination mode pour des vêtements et chaussures tendance au Maroc.
          </p>
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            {[['Accueil','/'],['Vêtements','/vetements'],['Chaussures','/chaussures'],['Accessoires','/produits']].map(([l,h]) => (
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            {[['Mon profil','/profil'],['Mes commandes','/commandes'],['Mon panier','/panier']].map(([l,h]) => (
              <li key={h}><Link to={h} className="hover:text-white transition-colors">{l}</Link></li>
            ))}
          </ul>
        </div>
